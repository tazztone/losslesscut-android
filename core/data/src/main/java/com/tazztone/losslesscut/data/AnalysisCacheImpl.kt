...
class AnalysisCacheImpl @Inject constructor(
    ...
    override fun getWaveform(clip: MediaClip, trackId: Int?): WaveformResult? = synchronized(lock) {
        val clipHash = getClipIdentityHash(clip)
        val file = File(cacheDir, "waveform_${clipHash}_t${trackId ?: 0}_v3.bin")
    ...
    override fun saveWaveform(clip: MediaClip, waveform: WaveformResult, trackId: Int?): Unit = synchronized(lock) {
        if (waveform.rawAmplitudes.isEmpty() || waveform.rawAmplitudes.size > MAX_WAVEFORM_SAMPLES) {
            return
        }
        val clipHash = getClipIdentityHash(clip)
        val targetFile = File(cacheDir, "waveform_${clipHash}_t${trackId ?: 0}_v3.bin")
        val tmpFile = File(cacheDir, "waveform_${clipHash}_t${trackId ?: 0}_v3.bin.tmp")
